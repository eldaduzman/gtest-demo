...
protected:
    ...
    float calculate_duration_percentile(int percentile)
    {
        ...
        std::chrono::nanoseconds indexed_duration_cron = thread_durations[sorted_array_index];
        float indexed_duration_milliseconds = (float)(indexed_duration_cron.count() / 1000000);
